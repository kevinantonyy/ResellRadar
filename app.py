...
USER_AGENT = "ResellRadar/0.2 (+local personal resale scanner)"
    ...
    with db() as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS scanner_hits (
                ...
                potential_roi REAL,
                status TEXT NOT NULL DEFAULT 'NEEDS COMPS',
        ...
        if not row:
            conn.execute(
                ...
                (
                    ...
                    DEFAULTS["scan_interval_min"], DEFAULTS["max_item_cost"],
                ),
            )
...
def extract_price(text: str) -> float | None:
    if not text:
        return None
    # Prefer dollar prices; reject obvious percentages and tiny coupon amounts.
...
def ebay_active_estimate(title: str) -> tuple[float | None, int]:
    """Conservative active-listing estimate. This is NOT sold-history data."""
    ...
    prices.sort()
    # Trim extremes, then use median. Asking prices can overstate realizable resale value.
    if len(prices) >= 8:
...
def find_comp_rule(title: str):
    title_l = title.lower()
    with db() as conn:
        rules = conn.execute("SELECT * FROM comp_rules ORDER BY LENGTH(keyword) DESC").fetchall()
    for rule in rules:
        if rule["keyword"].lower() in title_l:
            return rule
    return None


def score_scanner_hit(buy_price: float | None, estimate: float | None, fee_pct: float,
                      outbound_shipping: float, comp_count: int, comp_source: str | None, settings):
    if buy_price is None:
        return None, None, "NO PRICE"
    if estimate is None:
        return None, None, "NEEDS COMPS"
    ...
    # Only verified/manual SOLD comps can produce a BUY grade. Active asking prices are candidates only.
    verified = comp_source == "verified sold comp"
    liquid = comp_count >= 5
    if verified and liquid and profit >= settings["min_profit"] and roi >= settings["min_roi"]:
        status = "BUY"
...
def run_scan() -> int:
    ...
    try:
        ...
        for item in feed:
            ...
            rule = find_comp_rule(item["title"])
            estimate = None
            comp_count = 0
            comp_source = None
            ...
            if rule:
                ...
                try:
                    estimate, comp_count = ebay_active_estimate(item["title"])
                    if estimate is not None:
                        comp_source = "eBay active median"
                except Exception:
                    # One failed market lookup should not kill the whole feed scan.
                    estimate, comp_count, comp_source = None, 0, None

            profit, roi, status = score_scanner_hit(
                buy_price, estimate, fee_pct, outbound_shipping, comp_count, comp_source, settings
            )
            ...
            with db() as conn:
                ...
                if existing:
                    conn.execute(
                        ...
                        raw_description=? WHERE fingerprint=?""",
                        (now, item["title"], buy_price, estimate, comp_count, comp_source, fee_pct,
                         outbound_shipping, profit, roi, status, item["description"], fingerprint),
                    )
                else:
                    conn.execute(
                        """INSERT INTO scanner_hits
                        (fingerprint,first_seen,last_seen,title,source,deal_url,buy_price,market_estimate,comp_count,
                        comp_source,fee_pct,outbound_shipping,potential_profit,potential_roi,status,raw_description)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (fingerprint, now, now, item["title"], "Slickdeals", item["link"], buy_price, estimate,
                         comp_count, comp_source, fee_pct, outbound_shipping, profit, roi, status, item["description"]),
                    )
                    found += 1
...
def scanner_loop():
    # Initial scan shortly after startup, then use the dashboard-configured interval.
...
def index():
    ...
    with db() as conn:
        ...
        hits = conn.execute(
            ...
            ORDER BY CASE status WHEN 'BUY' THEN 1 WHEN 'CHECK SOLD' THEN 2 WHEN 'NEEDS COMPS' THEN 3 WHEN 'SKIP' THEN 4 ELSE 5 END,
            COALESCE(potential_profit,-9999) DESC, id DESC LIMIT 100"""
        ).fetchall()
        rules = conn.execute("SELECT * FROM comp_rules ORDER BY id DESC").fetchall()
    deals = []
    committed = 0.0
    for row in rows:
...
def scan_now():
    ...
    if err:
        ...
        flash(f"Scan complete. {found} new deal(s) added.", "ok")
...
def add_comp():
    ...
    with db() as conn:
        conn.execute(
            """INSERT INTO comp_rules
            (created_at,keyword,expected_sell_price,recent_sales,sell_marketplace,fee_pct,outbound_shipping,notes)
            VALUES (?,?,?,?,?,?,?,?)
            ON CONFLICT(keyword) DO UPDATE SET expected_sell_price=excluded.expected_sell_price,
            recent_sales=excluded.recent_sales,sell_marketplace=excluded.sell_marketplace,fee_pct=excluded.fee_pct,
    ...
    run_scan()
    flash("Verified comp rule saved and scanner rescored.", "ok")
    return redirect(url_for("index"))
...
def delete_comp(rule_id):
    with db() as conn:
        conn.execute("DELETE FROM comp_rules WHERE id=?", (rule_id,))
    flash("Comp rule deleted.", "ok")
    return redirect(url_for("index"))
...
def update_settings():
    ...
    with db() as conn:
        conn.execute(
            ...
            scan_interval_min=?, max_item_cost=? WHERE id=1""", vals,
        )
...
if __name__ == "__main__":
    ...
    t.start()
    # Disable Flask's reloader so the background scanner starts only once.
